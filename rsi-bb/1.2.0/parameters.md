...
form:
  ...
  fields:
    ...
    - id: require_price_improvement
      ...
      default: false
      help: "When on, a SELL only fires above the last BUY price and a BUY only below the last SELL price. The first trade in each direction has no reference and is unconstrained."
...
| `require_price_improvement` | Require price improvement | false | On: SELL only above the last BUY, BUY only below the last SELL. |
