...
form:
  ...
  fields:
    ...
    - id: require_price_improvement
      ...
      default: false
      help: "When on, a signal (after invert) must improve on both the last opposite-side fill (SELL above the last BUY, BUY below the last SELL) and — when the previous action was the same side — the last same-side fill (each consecutive SELL higher than the prior SELL, each consecutive BUY lower than the prior BUY). A trade with no relevant reference yet is unconstrained."
...
A two-moving-average crossover strategy that **opens and closes** one position: a
(post-`invert`) signal opposite the open position closes it, a matching signal
...
> worker's `step` path — see `strategy-core/src/ma_cross.rs` (`on_bar`).
