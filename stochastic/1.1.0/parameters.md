...
form:
  ...
  fields:
    ...
    - id: trade_pct
      label: "Trade % of capital"
      component: number
      bind: "params.trade_pct"
      default: 5
      help: "Percent of initial capital committed per signal. The dollar size adjusts to your capital."
      visible_when:
        trade_mode: ["percent"]
      validation:
        required: true
...
| `trade_mode`   | Trade size       | percent | `percent` of capital or a fixed `cash` amount.     |
| `trade_pct`    | Trade % capital  | 5       | Percent of capital per signal (when `mode=percent`).|
