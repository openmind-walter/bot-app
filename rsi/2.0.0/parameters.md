...
form:
  ...
  fields:
    ...
    - id: short_allowed
      ...
      lock_on_edit: true
      default: false
      help: "Off for spot: a SELL only closes an open long (down to flat) and from flat HOLDs. On for margin/futures: a SELL that closes a long also opens a short (and a BUY that closes a short opens a long), so the bot is always in the market once started."
...
On margin (`short_allowed` on) a close also **reverses** — the order flattens the
old side and opens the new one in a single fill. On spot (the default) a SELL can
only close a long: once flat it HOLDs until the next long. Opening trades are
sized to `trade_value` (capped by cash); a close/reverse carries the full held
quantity plus, when reversing, the new opening size.
...
| `short_allowed`| Allow short      | false   | Off (spot): a SELL only closes a long. On: closes reverse into the opposite side.|
...
> position short — it only closes an open long. Enable `short_allowed` for
> margin/futures, where closes reverse into the opposite side. The strategy
